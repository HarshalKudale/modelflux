/**
 * LocalLLMManager Component
 * 
 * This component shows a warning banner when a local provider is selected
 * but no model is loaded. It no longer handles dynamic model loading -
 * loading is now done manually from the provider settings screen.
 */

import React from 'react';
import { Platform, StyleSheet, Text, View } from 'react-native';
import { FontSizes, Spacing } from '../../../config/theme';
import { isLocalProvider, useConversationStore, useExecutorchLLMStore, useLLMStore } from '../../../state';

interface LocalLLMManagerProps {
    children: React.ReactNode;
}

/**
 * Main LocalLLMManager component
 * Shows warning banner if local provider is selected but no model is loaded
 */
export function LocalLLMManager({ children }: LocalLLMManagerProps) {
    const { selectedModelName, selectedModelId, isReady, isLoading, downloadProgress } = useExecutorchLLMStore();
    const { getConfigById } = useLLMStore();
    const { getCurrentConversation } = useConversationStore();

    // Get conversation's LLM config
    const conversation = getCurrentConversation();
    const conversationConfig = conversation?.activeLLMId ? getConfigById(conversation.activeLLMId) : null;

    // Check if we need a local model
    const needsLocalModel = conversationConfig && isLocalProvider(conversationConfig.provider);

    // Check if we have a local model ready
    const hasMatchingLoadedModel = selectedModelId && isReady;

    // Show warning on web for local providers
    if (Platform.OS === 'web' && needsLocalModel) {
        return (
            <View style={styles.container}>
                <View style={styles.errorBanner}>
                    <Text style={styles.errorText}>
                        Local models are not supported on web. Please use a native build (iOS/Android).
                    </Text>
                </View>
                {children}
            </View>
        );
    }

    // Show warning if local provider is selected but no model is loaded
    if (needsLocalModel && !hasMatchingLoadedModel && !isLoading) {
        return (
            <View style={styles.container}>
                <View style={styles.warningBanner}>
                    <Text style={styles.warningText}>
                        No local model loaded for this provider.
                        Load a model from provider settings to use this LLM.
                    </Text>
                </View>
                {children}
            </View>
        );
    }

    // Show loading banner if model is loading
    if (needsLocalModel && isLoading) {
        // Download progress is 0-1, multiply by 100 for percentage
        const progressPercent = Math.round(downloadProgress * 100);
        const statusText = downloadProgress < 1
            ? `Downloading model... ${progressPercent}%`
            : 'Loading model...';

        return (
            <View style={styles.container}>
                <View style={styles.loadingBanner}>
                    <Text style={styles.loadingText}>
                        {statusText}
                    </Text>
                </View>
                {children}
            </View>
        );
    }

    // Show loaded model info if ready
    if (needsLocalModel && hasMatchingLoadedModel) {
        return (
            <View style={styles.container}>
                <View style={styles.successBanner}>
                    <Text style={styles.successText}>
                        Using local model: {selectedModelName}
                    </Text>
                </View>
                {children}
            </View>
        );
    }

    // Otherwise, just render children (remote provider or no provider selected)
    return <>{children}</>;
}

const styles = StyleSheet.create({
    container: {
        flex: 1,
    },
    loadingBanner: {
        padding: Spacing.sm,
        backgroundColor: '#EEF2FF',
    },
    loadingText: {
        color: '#4338CA',
        fontSize: FontSizes.sm,
        textAlign: 'center',
    },
    warningBanner: {
        padding: Spacing.sm,
        backgroundColor: '#FEF3C7',
    },
    warningText: {
        color: '#92400E',
        fontSize: FontSizes.sm,
        textAlign: 'center',
    },
    errorBanner: {
        padding: Spacing.sm,
        backgroundColor: '#FEE2E2',
    },
    errorText: {
        color: '#991B1B',
        fontSize: FontSizes.sm,
        textAlign: 'center',
    },
    successBanner: {
        padding: Spacing.sm,
        backgroundColor: '#D1FAE5',
    },
    successText: {
        color: '#065F46',
        fontSize: FontSizes.sm,
        textAlign: 'center',
    },
});
